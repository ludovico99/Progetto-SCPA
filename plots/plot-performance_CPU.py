...
samplings_serial = [samplings_3_serial, samplings_4_serial, samplings_8_serial,
                    samplings_12_serial, samplings_16_serial, samplings_32_serial, samplings_64_serial]
...
def print_Gflops():
    ...
    for i in range(0, len(K)):
        ...
        gflops = compute_GFLOPS(K[i], nz, np.array(mean)*1e9)

        serial_mean = samplings_serial[i][0]
        serial_gflops = compute_GFLOPS(K[i], nz, np.array(serial_mean)*1e9)

        ax.scatter(1, serial_gflops, color=color_names[i])

        ax.plot(x, gflops, marker='o', markersize=2, label="Mean GFLOPS with K = {} and variable num_thread". format(
            K[i]), linewidth=0.5, color=color_names[i])

    ax.legend(loc='upper left', shadow=True, fontsize=10)

    plt.title("Matrice {}: Plot della media dei GFLOPS al variare del numero di threads e K".format(sys.argv[1]),
    ...
    # plt.savefig("Immagini/CSR_CPU_GFLOPS_{}.png".format(sys.argv[1]))
    plt.savefig("Immagini/ELLPACK_CPU_GFLOPS_{}.png".format(sys.argv[1]))
...
def print_all_results():
    ...
    for i in range(0, len(K)):
        ...
        ax.scatter(1, serial_mean, label="Mean value for serial product with K = {}".format(
            K[i]), color=color_names[i])

        ci = stats.t.interval(0.995, n-1, loc=serial_mean,
                              scale=std_serial/np.sqrt(n))
        ax.fill_between([1], ci[0], ci[1],
                        color=color_names[i], alpha=0.1)

        ax.plot(x, mean, marker='o', markersize=2, label="Mean value with K = {} and variable num_thread". format(
            K[i]), linewidth=0.5, color=color_names[i])
...
if len(sys.argv) >= 2:
    df_parallel = pandas.read_csv(
        "samplings_ELLPACK_CPU_parallel_{}.csv".format(sys.argv[1]))
...
if len(sys.argv) >= 2:
    df_serial = pandas.read_csv(
        "samplings_ELLPACK_CPU_serial_{}.csv".format(sys.argv[1]))
